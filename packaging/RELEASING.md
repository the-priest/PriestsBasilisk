...
```
```bash
git commit -m "v1.1.0.0 — leashed work mode, promise gate, native packages"
```
```bash
git tag -a v1.1.0.0 -m "v1.1.0.0"
```
```bash
...
first (`git push origin :refs/tags/v1.1.0.0`) — a moved tag that a release is
...
```bash
gh release create v1.1.0.0 --title "v1.1.0.0" --notes-file RELEASE_NOTES.md
```
```bash
gh release upload v1.1.0.0 priestsbasilisk_1.1.0.0-1_all.deb priestsbasilisk-1.1.0.0-1-any.pkg.tar.zst PriestsBasilisk-1.1.0.0.zip SHA256SUMS
```

To replace a file you already uploaded, add `--clobber`:

```bash
gh release upload v1.1.0.0 priestsbasilisk_1.1.0.0-1_all.deb --clobber
```

### By hand

Go to **Releases → Draft a new release**, choose the `v1.1.0.0` tag, paste the
...
| File | Who it is for |
| --- | --- |
| `priestsbasilisk_1.1.0.0-1_all.deb` | Kali, Debian, Ubuntu |
| `priestsbasilisk-1.1.0.0-1-any.pkg.tar.zst` | Arch, CachyOS |
| `PriestsBasilisk-1.1.0.0.zip` | source, for people who want to read it first |
...
```bash
sha256sum priestsbasilisk_1.1.0.0-1_all.deb priestsbasilisk-1.1.0.0-1-any.pkg.tar.zst PriestsBasilisk-1.1.0.0.zip > SHA256SUMS
```
...
```
```bash
git add PKGBUILD .SRCINFO && git commit -m "v1.1.0.0" && git push
```
...
```bash
mkdir -p apt-repo/pool/main && cp priestsbasilisk_1.1.0.0-1_all.deb apt-repo/pool/main/
```
```bash
...
`1.1.0.0` — if you cut `1.1.0.1`, these move with it:
...
```
```bash
git tag -a v1.1.0.0 -m "v1.1.0.0" && git push origin main --follow-tags
```
```bash
sha256sum priestsbasilisk_1.1.0.0-1_all.deb priestsbasilisk-1.1.0.0-1-any.pkg.tar.zst PriestsBasilisk-1.1.0.0.zip > SHA256SUMS
```
