...
- **Added an allow-listed `web_read` — the safe way back to reading a page.** It fetches only from a fixed allow-list of authoritative sources (`kali_core._WEB_READ_ALLOW`: NVD/NIST, MITRE, CISA, FIRST, official vendor/distro security channels, OWASP, PortSwigger, Kali docs, exploit-db). The host is matched on the parsed hostname (never a substring, so `nvd.nist.gov.evil.com` and userinfo tricks are rejected), **redirects are re-validated on every hop** (a trusted host can't 302 you off-list or into the local network), the final host is re-checked, and the output is run through `webshield`. Same discipline as `cve_lookup`: nothing — not the model, not a target that fed it a URL — can point it at attacker-authored content. The persona now tells the model to `web_read` an authoritative source when it's unsure of a CVE / flag / technique instead of guessing, and to say so (not invent) when something isn't on the list. It's a core tool (always available); core is still 3 tools.
