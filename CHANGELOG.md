...
- **The payload builders are general-purpose, not Juice-Shop-bound.** SSTi, SSRF, deserialization, prototype-pollution, path-traversal and XSS builders emit the universal techniques for any authorised target — the persona and docs now say so plainly. (The Juice-Shop-specific helpers — coupon/CAPTCHA/reset — are still there for the benchmark, clearly labelled as such.)
